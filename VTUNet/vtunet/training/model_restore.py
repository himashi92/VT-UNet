...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    """
    This is a utility function to load any vtunet trainer from a pkl. It will recursively search
    vtunet.trainig.network_training for the file that contains the trainer and instantiate it with the arguments saved in the pkl file. If checkpoint
    is specified, it will furthermore load the checkpoint file in train/test mode (as specified by train).
    The pkl file required here is the one that will be saved automatically when calling vtunetTrainer.save_checkpoint.
    :param pkl_file:
    :param checkpoint:
    :param train:
    ...
    name = info['name']
    if 'nnUNet' in name:
        name=name.replace('nnUNet','vtunet')
    if len(init)>10:
        init=list(init)
        del init[2]
        del init[-2]
    search_in = join(vtunet.__path__[0], "training", "network_training")
    tr = recursive_find_python_class([search_in], name, current_module="vtunet.training.network_training")
    ...
    if tr is None:
        raise RuntimeError("Could not find the model trainer specified in checkpoint in vtunet.trainig.network_training. If it "
                           "is not located there, please move it or change the code of restore_model. Your model "
                           "trainer can be located in any directory within vtunet.trainig.network_training (search is recursive)."
                           "\nDebug info: \ncheckpoint file: %s\nName of trainer: %s " % (checkpoint, name))
    assert issubclass(tr, vtunetTrainer) , "The network trainer was found but is not a subclass of vtunetTrainer.Please make it so!"

    # this is now deprecated
    """if len(init) == 7:
        print("warning: this model seems to have been saved with a previous version of vtunet. Attempting to load it "
...
if __name__ == "__main__":
    pkl = "/home/fabian/PhD/results/vtunetV2/vtunetV2_3D_fullres/Task004_Hippocampus/fold0/model_best.model.pkl"
    checkpoint = pkl[:-4]
    train = False
