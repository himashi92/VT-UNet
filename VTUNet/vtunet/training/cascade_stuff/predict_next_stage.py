...
import argparse
from vtunet.preprocessing.preprocessing import resample_data_or_seg
from batchgenerators.utilities.file_and_folder_operations import maybe_mkdir_p
import vtunet
from vtunet.run.default_configuration import get_default_configuration
...
if __name__ == "__main__":
    ...
    trainer_class = recursive_find_python_class([join(vtunet.__path__[0], "training", "network_training")],
                                                trainerclass,
                                                "vtunet.training.network_training")

    if trainer_class is None:
        raise RuntimeError("Could not find trainer class in vtunet.training.network_training")
    else:
        assert issubclass(trainer_class,
                          vtunetTrainer), "network_trainer was found but is not derived from vtunetTrainer"
