...
import vtunet
from vtunet.experiment_planning.summarize_plans import summarize_plans
from vtunet.paths import network_training_output_dir, preprocessing_output_dir, default_plans_identifier
...
def get_default_configuration(network, task, network_trainer, plans_identifier=default_plans_identifier,
                              search_in=(vtunet.__path__[0], "training", "network_training"),
                              base_module='vtunet.training.network_training'):
    ...
    if task == 'Task001_ACDC':
        print("Task ACDC here we go !!!")
        plans['plans_per_stage'][0]['batch_size'] = 2
        # plans['plans_per_stage'][0]['patch_size'] = np.array([16, 160, 160])
        plans['plans_per_stage'][0]['patch_size'] = np.array([16, 192, 192])
        pickle_file = open(plans_file, 'wb')
        pickle.dump(plans, pickle_file)
        pickle_file.close()
    elif task == 'Task002_Synapse':
        print("Task Synapse here we go !!!")
        plans['plans_per_stage'][1]['batch_size'] = 1
        plans['plans_per_stage'][1]['patch_size'] = np.array([64, 128, 128])
        plans['plans_per_stage'][1]['pool_op_kernel_sizes'] = [[2, 2, 2], [2, 2, 2], [2, 2, 2]]
        pickle_file = open(plans_file, 'wb')
        pickle.dump(plans, pickle_file)
        pickle_file.close()
    ...
    print("###############################################")
    print("I am running the following vtunet: %s" % network)
